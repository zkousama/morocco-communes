...
- [MCP server](${at("/mcp")}): Streamable HTTP, no key, with the tools search, get_commune, get_unit, communes_near, commune_at, list_communes, get_indicators, get_economy, get_housing and get_insights
...
- [Neighbours](${at("/api/communes/tiznit/neighbours")}): the communes one borders, with the length of the boundary each pair shares
...
- [Census indicators as CSV](${at("/data/v1/indicators/people.csv")}): every unit's figures about people, and [households](${at("/data/v1/indicators/households.csv")}); [fields.json](${at("/data/v1/indicators/fields.json")}) names each field with HCP's heading, unit and notes, and carries HCP's own definition of the 52 concepts it defines
...
- [Urban dwellings as CSV](${at("/data/v1/housing/dwellings.csv")}): the 2024 urban housing stock of every unit that has one, by occupancy, kind, age, materials and networks; [fields.json](${at("/data/v1/housing/fields.json")}) names each field with HCP's heading
